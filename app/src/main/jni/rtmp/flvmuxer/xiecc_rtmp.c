...
void * rtmp_sender_alloc() {
    ...
    rtmp_xiecc->audio_config_ok = false;
    rtmp_xiecc->video_config_ok = false;
...
int rtmp_open_for_write(void *handle, const char *url, uint32_t video_width, uint32_t video_height) {
    RTMP_XIECC *rtmp_xiecc = (RTMP_XIECC *)handle;
    RTMP *rtmp;
    if (rtmp_xiecc == NULL || rtmp_xiecc->rtmp == NULL) {
...
int rtmp_is_connected(void *handle)
{
    RTMP_XIECC *rtmp_xiecc = (RTMP_XIECC *)handle;
    if (rtmp_xiecc && rtmp_xiecc->rtmp) {
        if (RTMP_IsConnected(rtmp_xiecc->rtmp)) {
            return 1;
        }
    }
    return 0;
...
int rtmp_read_date(void *handle, uint8_t* data, int size) {
    RTMP_XIECC *rtmp_xiecc = (RTMP_XIECC *)handle;
    if (rtmp_xiecc && rtmp_xiecc->rtmp) {
        RTMP_Read(rtmp_xiecc->rtmp, data, size);
    }
    return 0;
}
                                  ...
                                  uint32_t abs_ts)
{
    ...
    if (rtmp_xiecc->audio_config_ok == false) {
        ...
        rtmp_xiecc->audio_config_ok = true;
    }
    else {
...
int send_key_frame(RTMP* rtmp, int nal_len,  uint32_t ts,  uint32_t abs_ts, uint8_t *nal, int totalLength, uint8_t **buf_offset, uint8_t *buf) {
    int offset = 0;
                                  ...
                                  uint32_t abs_ts)
{
    ...
    while (nal != NULL) {
        ...
        if ((nal[0] & 0x1f) == 0x07)  { // it can be 27,47,67
            if (rtmp_xiecc->video_config_ok == true) {
            ...
            rtmp_xiecc->video_config_ok = true;
