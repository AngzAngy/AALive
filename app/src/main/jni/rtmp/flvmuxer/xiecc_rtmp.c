...
//    rtmp->Link.timeout = 10; //10seconds
    ...
    if (RTMP_SUCCESS != RTMP_SetupURL(rtmp, (char *)url)) {
...
int rtmp_sender_start_publish(void *handle, uint32_t flag, int64_t ts_us)
{
    ...
    if (RTMP_SUCCESS != RTMP_Connect(rtmp, NULL) || RTMP_SUCCESS != RTMP_ConnectStream(rtmp, 0))  {
        return 1;
    }
...
// @brief rtmp has been connected
// @param [in] rtmp_sender handler
// @return             : 1: OK; others: FAILED
int rtmp_sender_is_connected(void *handle){
    RTMP_XIECC *rtmp_xiecc = (RTMP_XIECC *)handle;
    if (rtmp_xiecc == NULL || rtmp_xiecc->rtmp == NULL) {
        return 0;
    }
    return RTMP_IsConnected(rtmp_xiecc->rtmp) ? 1 : 0;
}
